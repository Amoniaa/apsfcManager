...
public class DBUtil {
	...
	public Object getObject(Connection conn, Class<?> type, String sql,Object[] paramList)  throws Exception {
		// 获取javaBean属性  
		BeanInfo beanInfo = Introspector.getBeanInfo(type);
		// 创建 JavaBean 对象  
		Object obj = type.newInstance();
		//获取Javabean中的所有属性,并给 JavaBean 对象的属性赋值   
		PropertyDescriptor[] propertyDescriptors = beanInfo.getPropertyDescriptors();
		Map	map = getObject(conn, sql, paramList);
		if(map != null){
			for (int i = 0; i < propertyDescriptors.length; i++) {
				PropertyDescriptor descriptor = propertyDescriptors[i];
				String propertyName = descriptor.getName();
	...
	public Map<String, Object> getObject(Connection conn, String sql, Object[] paramList) throws Exception {
		if (sql == null || sql.trim().equals("")) {
			logger.info("parameter is valid!");
			return null;
		}
